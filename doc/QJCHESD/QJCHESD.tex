...
\begin{enumerate}[label=\arabic*)]
    \item Update $q(\alpha)$
    	...
    	&= (a + Z -1)ln(\alpha) - \alpha(b+ \Vert \bm y_p - \bm\Phi_p \bm\mu_h\Vert^2 + \text{tr}(\bm\Phi_p^H\bm\Phi_p\bm\Sigma_h)) 
    	\end{align}
    	Therefore,
    	\begin{align}
    	a^{(t+1)} &= a^{(t)} + z \\
	...
	\begin{equation}
	\bm\Sigma_h^{(t+1)} = \hat{\alpha}^{(t+1)}\bm\Phi_h^H\bm\Phi_h + \text{diag}(\bm\gamma^{(t)})
	\end{equation}
	...
	\end{align}
	Please note that $\bm\Sigma_h^{(t+1)^H} = \bm\Sigma_h^{(t+1)}$
