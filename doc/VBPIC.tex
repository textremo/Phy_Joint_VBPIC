...
\subsubsection{Inital Channel Estimation}
In this section, we have two methods: the first, we average all channel estimation area together and do absolute square; the second, we do absolute square and average. We will introduce two thresholds individually.
\begin{itemize}
\item First
\begin{align}
Y_{CHE}[k,l] &= \frac{1}{N_b}\sum_{i=0}^{N_b-1} Y_{DD}[k,l+(l_\max+1)(i-1)]
\end{align}
where $k\in [0, K]$ and $l\in [0, l_\max]$ and the variance of $Y_{CHE}[k,l]$ is $\sigma_{CHE}^2=\frac{1+\sigma^2}{N_b}$
\begin{align}
| \eta[k,l] |^2 &= | Y_{CHE}[k,l] |^2 \\
\frac{|\eta[k,l]|^2}{\sigma_{CHE}^2/2} &=  |\frac{real(Y_{CHE}[k,l])}{\sigma_{CHE}/\sqrt(2)}|^2 + |\frac{imag(Y_{CHE}[k,l])}{\sigma_{CHE}/\sqrt(2)}|^2
\end{align}
Therefore, $\frac{|\eta[k,l]|^2}{\sigma_{CHE}^2/2}$ is a chi-square distribution of freedom 2. We set the a threshold $\rho_0$ and its probability $p_0$ (assuming it is noise and data), i.e.,
\begin{align}
p(\frac{|\eta[k,l]|^2}{\sigma_{CHE}^2/2} \leq \rho_0) &= p_0 \\
\rho_0 &= chi2inv(p_0, 2) \\
\frac{|\eta[k,l]|^2}{\sigma_{CHE}^2/2} &\leq \rho_0 \\
|\eta[k,l]|^2 &\leq \rho_0 \sigma_{CHE}^2/2 \\
|\eta[k,l]|^2 &\leq \rho_0 \frac{1+\sigma^2}{2N_b}
\end{align}
If $|\eta[k,l]|^2 \geq \rho_0 \frac{1+\sigma^2}{2N_b}$, we assume that it is not noise and data at a probability of $1-p_0$.
Then, we set another threshold $\rho_1$ and its probability $p_1$ (assuming it is a path where $\sigma_{CHE}^2=\frac{1+\sigma^2}{N_b}$), i.e.,
\begin{align}
p(|\eta[k,l]|^2 \geq \rho_1) &\geq p_1  \\
p(|\eta[k,l]|^2 \leq \rho_1) &\leq 1-p_1  \\
\rho_1 &= chi2inv(1-p_1, 2) \\
\frac{|\eta[k,l]|^2}{(E_s/p + (1+\sigma^2)/N_b)/2} &\geq \rho_1 \\
|\eta[k,l]|^2 &\geq \rho_1\frac{E_s/p + (1+\sigma^2)/N_b}{2}
\end{align}
To make sure that a path exist when it is not a noise
\begin{align}
\rho_0 \frac{1+\sigma^2}{2N_b} &= \rho_1\frac{E_s/p + (1+\sigma^2)/N_b}{2}  \\
\frac{\rho_0}{\rho_1} \frac{1+\sigma^2}{N_b} &= E_s/p + (1+\sigma^2)/N_b  \\
E_s/p &= (\frac{\rho_0}{\rho_1} -1) \frac{1+\sigma^2}{N_b}
\end{align}

\item Second
\begin{align}
|\eta[k,l]|^2 &= \sum_{i=0}^{N_b-1} |Y_{DD}[k,l+(l_\max+1)(i-1)]|^2 \\
\frac{|\eta[k,l]|^2}{(1+\sigma^2)/2}&= \sum_{i=0}^{N_b-1} \frac{real(Y_{DD}[k,l+(l_\max+1)(i-1)])^2}{(1+\sigma^2)/2} + \frac{imag(Y_{DD}[k,l+(l_\max+1)(i-1)])^2}{(1+\sigma^2)/2}
\end{align}
Therefore, $\frac{|\eta[k,l]|^2}{(1+\sigma^2)/2}$ is a chi-square distribution of freedom $2N_b$. We set the a threshold $\rho_0$ and its probability $p_0$ (assuming it is noise and data), i.e.,
\begin{align}
p(\frac{|\eta[k,l]|^2}{(1+\sigma^2)/2} \leq \rho_0) &= p_0 \\
\rho_0 &= chi2inv(p_0, 2N_b) \\
|\eta[k,l]|^2 &\leq \rho_0 (1+\sigma^2)/2
\end{align}
Similarly,
\begin{align}
\rho_1 &= chi2inv(1-p_1, 2N_b) \\
|\eta[k,l]|^2 &\geq \rho_1\frac{E_s/p + 1 +\sigma^2}{2}
\end{align}
Therefore,
\[
E_s/p = (\frac{\rho_0}{\rho_1} -1) (1+\sigma^2)
\]
...
\end{enumerate}
\subsubsection{OTFS Channel Estimation using VB known channel positions}
...
\subsubsection{Enhanced Simplified OTFS Channel Estimation using VB}
Now, we assume $p(h_i)\sim \mathcal{CN}(\beta_i, \gamma_i^{-1})$ and $p(\beta_i)\sim \mathcal{CN}(\mu_{\beta_i}, \sigma^2_{\beta_i})$. Therefore,
\[
p(y,\Theta) = p(y|h,\alpha)p(y|\beta, \gamma)p(\beta)p(\gamma)
\]
\begin{enumerate}
\item \colorbox{yellow}{Update $q(h)$}
\begin{align}
	q^{(t+1)}(h) &\propto exp( \mathbb{E}_{q^{(t)}_{-h}}[\text{ln}p(y,\Theta)] \} ) \\
	lnq^{(t+1)}(h) &\propto \mathbb{E}_{q^{(t)}_{-h}}\{\text{ln}p(y,\Theta)\} \\
	&\propto \mathbb{E}_{q^{(t)}_{-h}}\{lnp(y|h,\alpha)\} + \mathbb{E}_{q^{(t)}_{-h}}\{lnp(h|\beta^{t},\gamma^{(t)})\} \\
	&\propto -\alpha\Vert \bm y_p-\bm\Phi_p\bm h \Vert^2 - (\bm h -\beta^{(t)})^H\text{diag}(\bm\gamma^{(t)})(\bm h - \beta^{(t)}) \\
	\end{align}    
	First, we open the left
	\begin{equation}
	-\alpha\Vert \bm y_p-\bm\Phi_p\bm h \Vert^2 \propto \alpha\bm h^H\bm\Phi_p^H\bm y_p + \alpha\bm y_p^H\bm\Phi_p\bm h - \alpha\bm h^H\bm\Phi_h^H\bm\Phi_h\bm h
	\end{equation}
	Then, we open the right
	\begin{equation}
	(\bm h -\beta^{(t)})^H\text{diag}(\bm\gamma^{(t)})(\bm h - \beta^{(t)}) \propto \bm \beta^{(t)^H}\text{diag}(\bm\gamma^{(t)})\bm h + \bm h^H\text{diag}(\bm\gamma^{(t)})\bm\beta^{(t)} - \bm h^H\text{diag}(\bm\gamma^{(t)})\bm h
	\end{equation}
	Therefore,
	\begin{align}
	q^{(t+1)}(h) &\propto h^H(\alpha\bm\Phi_p^H\bm y_p + \text{diag}(\bm\gamma^{(t)})\bm\beta^{(t)}) + \bm h^H(\alpha\bm\Phi_h^H\bm\Phi_h + \text{diag}(\bm\gamma^{(t)}))\bm h
	\end{align}
		Here, we can see that the covariance is 
	\begin{align}
	\bm\Sigma_h^{(t+1)} &= \left(\alpha\bm\Phi_h^H\bm\Phi_h + \text{diag}(\bm\gamma^{(t)})\right)^{-1} \\
	\bm\mu^{(t+1)} &= \bm\Sigma_h^{(t+1)} (\alpha\bm\Phi_p^H\bm y_p + \text{diag}(\bm\gamma^{(t)})\bm\mu_{\beta}^{(t)} )
	\end{align}
\item \colorbox{yellow}{Update $q(\gamma)$}
	\begin{align}
	\text{ln}q^{(t+1)}(\gamma) &\propto lnp(h|\beta,\gamma) + lnp(\gamma)\\
	&\propto \sum_{l=0}^{P_\max}\left( \text{ln}(\gamma_l) - \gamma_l\Vert h_l^{(t+1)} -\beta_l \Vert^2 + (c-1)\text{ln}\gamma_l -d\gamma_l \right) \\
	&\propto \sum_{l=0}^{P_\max}(c+1 -1)\text{ln}\gamma_l - \gamma_l(d + \Vert h_l^{(t+1)} -\beta_l \Vert^2 ) \\
	&\propto \sum_{l=0}^{P_\max}(c+1 -1)\text{ln}\gamma_l - \gamma_l(d + \Vert h_l^{(t+1)}\Vert^2  - \beta_l^H h -h_l^H\beta_l + \Vert\beta_l\Vert^2 ) \\
	&\propto \sum_{l=0}^{P_\max}(c+1 -1)\text{ln}\gamma_l - \gamma_l(d + \Vert h_l^{(t+1)}\Vert^2  - \mu_{\beta_l}^H \mu_l -{\mu_l}^H \mu_{\beta_l} + \Vert\beta_l\Vert^2 ) \\
	\end{align}
	where $\Vert h_l^{(t+1)} \Vert^2 = \Sigma_{h_l}^{t+1} + |\mu_{h_l}^{t+1}|^2$, $\Vert\beta_l\Vert^2 =\sigma_{\beta_l}^2 + |\mu_{\beta_l}^{(t+1)}|^2$


\item \colorbox{yellow}{Update $q(\beta)$}
\begin{align}
\text{ln}q^{(t+1)}(\beta) &\propto lnp(h|\beta,\gamma) + lnp(\beta) \\
&\propto \sum_{l=0}^{P_\max}\left( - \gamma_l\Vert h_l^{(t+1)} -\beta_l \Vert^2 - \sigma_{\beta_l}^{-2}\Vert \beta_l - \mu_{\beta_l} \Vert^2 \right) \\
&\propto \sum_{l=0}^{P_\max} -(\gamma_l + \sigma_{\beta_l}^{-2})\beta_l^H\beta_l +\beta_l^H(\gamma_l\mu_{h_l} + \sigma_{\beta_l}^{-2}\mu_{\beta_l} )\\
\end{align}
Therefore,
\begin{align}
\sigma_{\beta_l}^{{(t+1)}^2} &= (\gamma_l + \sigma_{\beta_l}^{-2})^{-1} \\
\mu_{\beta_l} &= \sigma_{\beta_l}^{{(t+1)}^2}( \gamma_l\mu_{h_l} + \sigma_{\beta_l}^{-2}\mu_{\beta_l}  ) 
\end{align}
